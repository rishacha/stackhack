...
const isTaskValid = (userDTO) => {
    ...
    const schema = {
        ...
        "dueDate": Joi.date().greater(Joi.ref('creationDate')).greater(Date.now()),// Date.now(), // timestamp
        "creationDate":Joi.date().required(),//Date.now(), // timestamp
...
const isUpdateValid = (userDTO) => {
    ...
    const schema = {
        ...
        // "userId":Joi.string().required(), // "u1", // string
        "taskStatus":Joi.number().min(0).max(2), //0, // 0 - New, 1 - In-Progress, 2 - Completed (int)
        "dueDate": Joi.date().greater(Date.now()),// Date.now(), // timestamp
